            ...
            onConnectCb: async () => {
                try {
                    ...
                    node.on("input", function (msg, send, done) {
                        c.readMessages(node.stream.streamName, new ReadMessagesOptions(msg.desiredStartSequenceNumber || msg.nextSequenceNumber || 0, 
                            ...
                            .catch((err) => { 
                                if (err.toString() === "Error: not enough messages to return before time out") {
                                    const lastMsg = {
                                        nextSequenceNumber: msg.desiredStartSequenceNumber,
                                        eos: true
                                    }
                                    send([[null], [lastMsg], [null]]);
                                    done();
                                    return;
                                }
